<template>
	<div class="font_ui">
		<h1 class="font_display m_0 lh_2 font_3">
			{{ participation.title }}
		</h1>
		<p class="c_primary m-t_0 lh_0 font_italic">
			{{ participation.subtitle }}
		</p>
			<Accordion
				v-for="(category, index) in sort"
				:key="index+'_group'"
				class="transition_1"
			>
				<template v-slot:header>
					<h2 class="font_display m_0 lh_2 font_2">
						<span class="lh_0" v-if="category != null">{{
							category
						}}</span>
						<span class="lh_0" v-if="category == null"
							>General</span
				...
				<template v-slot:content>
					<div class="font_copy p_4:md">
						...
						>
							...
							<div class="flex_auto w_30:md p_3">
								Participation
							</div>
							<div class="flex_auto w_40:md p_3">
								Associated Services
							</div>
						</div>
						<div
							class="br-b_1 br_solid br_black-2"
							v-for="(metric, index) in dataSorted.filter(
						...
						>
							<section
								class="flex flex_column flex_row:md br-t_1 br_solid br_black-1 p-x_3 p-y_2 p_0:md"
							>
								<div
